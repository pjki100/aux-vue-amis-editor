...
import { qsparse, JSONTraverse, qsstringify, hasFile, object2formData, uuid, isObjectShallowModified } from './helper.js';
...
function buildApi(api, data, options) {
    ...
    if (~idx) {
        ...
        api.url =
            left +
                (~left.indexOf('?') ? '&' : '?') +
                qsstringify((api.query = dataMapping(params, data, undefined, api.convertKeyToPath))) +
                (~hashIdx && hashIdx > idx
    ...
    if (api.method === 'get' || api.method === 'jsonp' || api.method === 'js') {
        if (!api.data &&
            ...
            if (~idx_1) {
                var params = (api.query = __assign(__assign({}, qsparse(api.url.substring(idx_1 + 1))), data));
                api.url = api.url.substring(0, idx_1) + '?' + qsstringify(params);
            }
            else {
                api.query = data;
        ...
        if (api.data && api.attachDataToQuery !== false) {
            ...
            if (~idx_2) {
                var params = (api.query = __assign(__assign({}, qsparse(api.url.substring(idx_2 + 1))), api.data));
                api.url = api.url.substring(0, idx_2) + '?' + qsstringify(params);
            }
            else {
                api.query = api.data;
